...
- **多摄像头**：双重枚举 —— `CameraManager.cameraIdList` 拿顶层逻辑相机，再用 CameraX 的 `CameraInfo.physicalCameraInfos` 拿每个逻辑相机底下的物理子摄像头（覆盖 Mi 9/MIUI 把超广角隐藏在主摄逻辑相机里的情况）。物理子摄像头通过 `Camera2Interop.Extender.setPhysicalCameraId()` 切换
...
- **物理子摄像头**：`Camera2Interop.setPhysicalCameraId()` 是实验 API，部分 OEM 可能拒绝该 ID 与 `VideoCapture` 组合；切换失败时日志会有 `bindWith failed` —— 那种情况下用不带物理 ID 的逻辑相机条目即可。Mi 9 实测可用
